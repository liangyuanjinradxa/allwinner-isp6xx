...
typedef enum {
	...
	HW_ISP_CFG_TUNING_FPN_COMM                 = 0x00040000,
	HW_ISP_CFG_TUNING_ENCPP_TOP_COMM           = 0x00080000,
	HW_ISP_CFG_TUNING_ENCPP_LDCI_COMM          = 0x00100000,
	HW_ISP_CFG_TUNING_NRP_COMM                 = 0x00200000,
	HW_ISP_CFG_TUNING_LCA                      = 0x00400000,
...
typedef enum {
	...
	HW_ISP_CFG_TUNING_ENCPP_TOP                = 0x00008000,
	HW_ISP_CFG_TUNING_ENCPP_LDCI               = 0x00010000,
	HW_ISP_CFG_TUNING_FPN                      = 0x00020000,
	HW_ISP_CFG_TUNING_GCA_TBL                  = 0x00040000,
	HW_ISP_CFG_TUNING_DPC_TBL                  = 0x00080000,
...
struct isp_tuning_encpp_top_comm_cfg {
	HW_U8		value[ENCPP_TOP_COMM_MAX];
};

struct isp_tuning_encpp_ldci_comm_cfg {
	HW_U8		value[ENCPP_LDCI_COMM_MAX];
};

struct isp_tuning_nrp_comm_cfg {
	HW_S16		value[ISP_NRP_COMM_MAX];
};

struct isp_tuning_lca_comm_cfg {
	HW_U16		value[ISP_LCA_COMM_MAX];
};
...
struct isp_tuning_bdnf_item_cfg {
	HW_S16		lp0_thres[ISP_REG_TBL_LENGTH_32];
	HW_S16		lp1_thres[ISP_REG_TBL_LENGTH_32];
	HW_S16		lp2_thres[ISP_REG_TBL_LENGTH_32];
};

struct isp_tuning_bdnf_table_cfg {
	struct isp_tuning_bdnf_item_cfg thres[ISP_DYNAMIC_GROUP_COUNT];
	HW_U16		cbth[ISP_REG_TBL_LENGTH_32];
	HW_U16		crth[ISP_REG_TBL_LENGTH_32];
	HW_U8		lp0_y_by_k[ISP_REG_TBL_LENGTH_16];
	HW_U8		lp1_y_by_k[ISP_REG_TBL_LENGTH_16];
	HW_U8		lp2_y_by_k[ISP_REG_TBL_LENGTH_16];
};

struct isp_tuning_tdnf_table_cfg {
	HW_S16      thres[ISP_DYNAMIC_GROUP_COUNT][ISP_REG_TBL_LENGTH_32];
	HW_U8		df_shape[ISP_REG_TBL_LENGTH_16];
	HW_U8		ratio_amp[ISP_REG_TBL_LENGTH_16];
	HW_U8		k_dlt_bk[ISP_REG_TBL_LENGTH_16];
	HW_U8		ct_rt_bk[ISP_REG_TBL_LENGTH_16];
	HW_U8		dtc_hf_bk[ISP_REG_TBL_LENGTH_16];
	HW_U8		dtc_mf_bk[ISP_REG_TBL_LENGTH_16];
...
struct isp_tuning_sharp_table_cfg {
	HW_U16		hs_value[ISP_REG_TBL_LENGTH_32];
	HW_U16		ms_value[ISP_REG_TBL_LENGTH_32];
	HW_U16		ls_value[ISP_REG_TBL_LENGTH_32];
	HW_U16		hs_lum[ISP_REG_TBL_LENGTH_32];
	HW_U16		ms_lum[ISP_REG_TBL_LENGTH_32];
	HW_U16		ls_lum[ISP_REG_TBL_LENGTH_32];
	HW_U16		hsv[46];
	HW_U8		ratio_by_mot[ISP_SHARP_RT_TBL_SIZE];
	HW_U8		ratio_by_tex[ISP_SHARP_RT_TBL_SIZE];
};
...
struct isp_tuning_pltm_table_cfg {
	HW_U8		stat_gd_cv[15];
	HW_U8		df_cv[ISP_REG_TBL_LENGTH_33];
	HW_U8		lum_map_cv[ISP_PLTM_LM_TBL_SIZE];
	HW_U16		gtm_tbl[ISP_PLTM_GTM_TBL_SIZE];
};
...
struct isp_tuning_lca_table_cfg {
	HW_U8 pf_satu_lut[ISP_REG_TBL_LENGTH_33];
	HW_U8 gf_satu_lut[ISP_REG_TBL_LENGTH_33];
};

struct isp_tuning_encpp_sharp_table_cfg {
	HW_U16		hs_value[ISP_REG_TBL_LENGTH_32];
	HW_U16		ms_value[ISP_REG_TBL_LENGTH_32];
	HW_U16		hs_lum[ISP_REG_TBL_LENGTH_32];
	HW_U16		ms_lum[ISP_REG_TBL_LENGTH_32];
	HW_U16		hsv[46];
	HW_U8		ls_map_lut[ISP_REG_TBL_LENGTH_32];
	HW_U8		texture_lut[ISP_REG_TBL_LENGTH_32];
	HW_U8		ratio_by_mot[ISP_SHARP_RT_TBL_SIZE];
	HW_U8		ratio_by_tex[ISP_SHARP_RT_TBL_SIZE];
};

struct isp_tuning_encpp_top_table_cfg {
	HW_U16		gbl_satu_adj_lut[ISP_REG_TBL_LENGTH_16];
};

struct isp_tuning_encpp_ldci_table_cfg {
	HW_U8		txt_ratio_lut[ISP_REG_TBL_LENGTH_32];
	HW_U8		mot_ratio_lut[ISP_REG_TBL_LENGTH_32];
	HW_U8		lum_diff_lut[ISP_REG_TBL_LENGTH_32];
	HW_U8		map_pos_lut[ISP_REG_TBL_LENGTH_16];
	HW_U8		map_neg_lut[ISP_REG_TBL_LENGTH_16];
};

struct isp_tuning_fpn_table_cfg {
	HW_U8		cfpn_coeff_lut[ISP_REG_TBL_LENGTH_17];
	HW_U8		pfpn_scale_coeff_lut[ISP_PFPN_TBL_SIZE];
	HW_U8		cfpn_gain_corr_tbl[ISP_CFPN_GAIN_CORR_TBL_SIZE];
};

struct isp_tuning_gca_table_cfg {
	HW_S8		hor_r_offset[825];
	HW_S8		ver_r_offset[825];
	HW_S8		hor_b_offset[825];
	HW_S8		ver_b_offset[825];
};

struct isp_tuning_dpc_table_cfg {
	HW_U32		static_dead_pix_tbl[1024];
};
...
struct isp_tuning_param_cfg {
	...
	struct isp_tuning_encpp_sharp_comm_cfg  encpp_sharp_comm; /* id: 0x0300010000 */
	struct isp_tuning_sensor_temp_cfg       sensor_temp;      /* id: 0x0300020000 */
	struct isp_tuning_fpn_comm_cfg          fpn;              /* id: 0x0300040000 */
	struct isp_tuning_encpp_top_comm_cfg    encpp_top_comm;   /* id: 0x0300080000 */
	struct isp_tuning_encpp_ldci_comm_cfg   encpp_ldci_comm;  /* id: 0x0300100000 */
	struct isp_tuning_nrp_comm_cfg          nrp_comm;         /* id: 0x0300200000 */
	struct isp_tuning_lca_comm_cfg          lca;              /* id: 0x0300400000 */
	/* tuning tables */
	...
	struct isp_tuning_lca_table_cfg         lca_tbl;          /* id: 0x0400002000 */
	struct isp_tuning_encpp_sharp_table_cfg encpp_sharp;      /* id: 0x0400004000 */
	struct isp_tuning_encpp_top_table_cfg   encpp_top;        /* id: 0x0400008000 */
	struct isp_tuning_encpp_ldci_table_cfg  encpp_ldci;       /* id: 0x0400010000 */
	struct isp_tuning_fpn_table_cfg         fpn_tbl;          /* id: 0x0400020000 */
	struct isp_tuning_gca_table_cfg         gca_tbl;          /* id: 0x0400040000 */
	struct isp_tuning_dpc_table_cfg         dpc_tbl;          /* id: 0x0400080000 */
};
...
struct isp_dynamic_sharp_dir_hs_item {
	HW_S16		value[ISP_SHARP_NDIR_HS_EDGE_TH];
};

struct isp_dynamic_sharp_ndir_hs_item {
	HW_S16		value[ISP_SHARP_DIR_MS_DTH_EDGE_TH - ISP_SHARP_NDIR_HS_EDGE_TH];
};

struct isp_dynamic_sharp_dir_ms_item {
	HW_S16		value[ISP_SHARP_NDIR_MS_EDGE_TH - ISP_SHARP_DIR_MS_DTH_EDGE_TH];
};

struct isp_dynamic_sharp_ndir_ms_item {
	HW_S16		value[ISP_SHARP_DIR_LS_DTH_EDGE_TH - ISP_SHARP_NDIR_MS_EDGE_TH];
};

struct isp_dynamic_sharp_dir_ls_item {
	HW_S16		value[ISP_SHARP_NDIR_LS_EDGE_TH - ISP_SHARP_DIR_LS_DTH_EDGE_TH];
};

struct isp_dynamic_sharp_ndir_ls_item {
	HW_S16		value[ISP_SHARP_NDIR_HS_MIX_LW_CLIP - ISP_SHARP_NDIR_LS_EDGE_TH];
};
...
struct isp_dynamic_sharp_cfg {
	HW_S16		trigger;
	struct isp_dynamic_sharp_dir_hs_item tuning_dir_hs_cfg[ISP_DYNAMIC_GROUP_COUNT];
	struct isp_dynamic_sharp_ndir_hs_item tuning_ndir_hs_cfg[ISP_DYNAMIC_GROUP_COUNT];
	struct isp_dynamic_sharp_dir_ms_item tuning_dir_ms_cfg[ISP_DYNAMIC_GROUP_COUNT];
	struct isp_dynamic_sharp_ndir_ms_item tuning_ndir_ms_cfg[ISP_DYNAMIC_GROUP_COUNT];
...
struct isp_dynamic_denoise_dnr_item {
	HW_S16		value[ISP_DENOISE_DTC_HF_WHT_STR];
};

struct isp_dynamic_denoise_dtc_item {
	HW_S16		value[ISP_DENOISE_LYR0_DNR_LM_AMP - ISP_DENOISE_DTC_HF_WHT_STR];
};
...
struct isp_dynamic_encpp_sharp_dir_hs_item {
	HW_S16		value[ENCPP_SHARP_NDIR_HS_EDGE_TH];
};

struct isp_dynamic_encpp_sharp_ndir_hs_item {
	HW_S16		value[ENCPP_SHARP_DIR_MS_DTH_EDGE_TH - ENCPP_SHARP_NDIR_HS_EDGE_TH];
};

struct isp_dynamic_encpp_sharp_dir_ms_item {
	HW_S16		value[ENCPP_SHARP_NDIR_MS_EDGE_TH - ENCPP_SHARP_DIR_MS_DTH_EDGE_TH];
};

struct isp_dynamic_encpp_sharp_ndir_ms_item {
	HW_S16		value[ENCPP_SHARP_NDIR_HS_MIX_LW_CLIP - ENCPP_SHARP_NDIR_MS_EDGE_TH];
};
...
struct isp_dynamic_encpp_sharp_cfg {
	HW_S16		trigger;
	struct isp_dynamic_encpp_sharp_dir_hs_item tuning_dir_hs_cfg[ISP_DYNAMIC_GROUP_COUNT];
	struct isp_dynamic_encpp_sharp_ndir_hs_item tuning_ndir_hs_cfg[ISP_DYNAMIC_GROUP_COUNT];
...
struct isp_dynamic_encpp_ldci_item {
	HW_S16		value[ENCPP_LDCI_MAX];
};

struct isp_dynamic_encpp_ldci_cfg {
	HW_S16		trigger;
...
struct isp_dynamic_param_cfg {
	...
	struct isp_dynamic_encpp_sharp_cfg      encpp_sharp;            /* id: 0x0500040000 */
	struct isp_dynamic_wdr_cfg              wdr;                    /* id: 0x0500080000 */
	struct isp_dynamic_encoder_denoise_cfg  encoder_denoise;        /* id: 0x0500100000 */
