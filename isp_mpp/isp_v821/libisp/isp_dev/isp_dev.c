...
#define SUNXI_TDM_RX "sunxi_tdm_rx"
...
static int isp_stat_req_buffers(struct hw_isp_device *isp)
{
	...
	if (config.config_counter == 1) /* use config_counter from kernel to malloc more buf to ldci */
		isp->size = config.buf_size + ISP_SAVE_LOAD_STATISTIC_SIZE + LDCI_PIC_WIDTH * LDCI_PIC_HEIGHT;
	else
...
static void isp_tdm_process_buffer(void *priv)
{
	struct hw_isp_device *isp = priv;
	struct v4l2_event event;
	struct vin_isp_tdm_event_status *status = (struct vin_isp_tdm_event_status *)event.u.data;
	struct vin_isp_tdm_event_status state;
	int ret;
	unsigned int i, j;

	memset(&event, 0, sizeof event);
	ret = ioctl(isp->tdm_rx.fd, VIDIOC_DQEVENT, &event);
	if (ret < 0) {
		ISP_ERR("unable to retrieve tdm event: %s (%d).\n",
			strerror(errno), errno);
		return;
	}

	//ISP_PRINT("isp%d get buf is %d, buf_iommu_addr is 0x%p, buf_size is %d, callback=%p\n", isp->id, status->buf_id, status->iommu_buf, status->buf_size, isp->isp_tdm_buffer_process);
	memcpy(&state, status, sizeof(struct vin_isp_tdm_event_status));
	if (isp->isp_tdm_buffer_process) {
		isp->isp_tdm_buffer_process(&state);
	} else {
		ret = ioctl(isp->tdm_rx.fd, VIDIOC_VIN_TDM_DQBUF, &state);
		if (ret)
			ISP_ERR("VIDIOC_VIN_TDM_DQBUF error!\n");
	}
}
...
{
	int ret = -1;
	ret = ioctl(isp->tdm_rx.fd, VIDIOC_VIN_TDM_DQBUF, status);
	if (ret)
		ISP_ERR("VIDIOC_VIN_TDM_DQBUF error!\n");
...
static int isp_tdm_open(struct hw_isp_device *isp)
{
	struct media_entity *entity = NULL;
	struct hw_isp_media_dev *media = isp->priv;
	char name[32];

	snprintf(name, sizeof(name), SUNXI_TDM_RX".%u", isp->id);
	ISP_DEV_LOG(ISP_LOG_SUBDEV, "tdm_rx device name is %s\n", name);
	ISP_PRINT("tdm_rx device name is %s\n", name);
	entity = media_get_entity_by_name(media->mdev, name);
	if (entity == NULL) {
		isp->tdm_rx.fd = -1;
		return -ENOENT;
	...
	return v4l2_subdev_open(&isp->tdm_rx);
}

static void isp_tdm_close(struct hw_isp_device *isp)
{
	v4l2_subdev_close(&isp->tdm_rx);
}
...
int isp_dev_open(struct hw_isp_media_dev *isp_md, int id)
{
	...
	ret = isp_tdm_open(isp);
	if (ret == -ENOENT) {
		ISP_WARN("ISP%d maybe online mode, ignore tdm open failed.\n", isp->id);
	} else if (ret < 0) {
		ISP_ERR("ISP%d open tdm video failed!\n", isp->id);
		goto free_isp;
	}
...
void isp_dev_close(struct hw_isp_media_dev *isp_md, int id)
{
	...
	isp_tdm_close(isp_md->isp_dev[id]);
	...
	ISP_PRINT("close isp device[%d] success!\n", id);
}
...
int isp_dev_start(struct hw_isp_device *isp)
{
	...
	/* Start the tdm raw data engine. */
	if (isp->tdm_rx.fd != -1) {
		memset(&esub, 0, sizeof(struct v4l2_event_subscription));
		esub.id = 0;
		esub.type = V4L2_EVENT_VIN_TDM;
		ret = ioctl(isp->tdm_rx.fd, VIDIOC_SUBSCRIBE_EVENT, &esub);
		if (ret < 0) {
			ISP_ERR("unable to subscribe to tdm event: %s (%d).\n", strerror(errno), errno);
		} else {
			isp->ops->monitor_fd(isp->id, isp->tdm_rx.fd, HW_ISP_EVENT_EXCEPTION, isp_tdm_process_buffer, isp);
		}
	}
...
int isp_dev_stop(struct hw_isp_device *isp)
{
	...
	if (isp->tdm_rx.fd != -1) {
		isp->ops->unmonitor_fd(isp->id, isp->tdm_rx.fd);

		memset(&esub, 0, sizeof esub);
		esub.type = V4L2_EVENT_VIN_TDM;
		ioctl(isp->tdm_rx.fd, VIDIOC_UNSUBSCRIBE_EVENT, &esub);
	}
...
void isp_get_debug_info(struct hw_isp_device *isp)
{
	...
	ctx->debug_param_info.gain_val = ctx->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_total_gain >> 4;
	...
	if ((ctx->awb_entity_ctx.awb_result.wb_gain_output.gr_gain != 0) && (ctx->awb_entity_ctx.awb_result.wb_gain_output.gb_gain != 0)) {
...
int isp_set_isp_reset(struct hw_isp_device *isp)
{
	...
	ret = ioctl(isp->subdev.fd, VIDIOC_VIN_ISP_RESET, &enable);
	if (ret)
		ISP_ERR("VIDIOC_VIN_ISP_RESET error! ret=%d\n", ret);
	return ret;
}
...
int isp_set_tmd_addr_map(struct hw_isp_device *isp, struct isp_tdm_map_cfg *map)
{
	...
	if (isp->tdm_rx.fd == -1){
		ISP_ERR("%s err, tdm rx is not open\n", __func__);
		return -1;
	}

	ret = ioctl(isp->tdm_rx.fd, VIDIOC_VIN_TDM_MAP, map);
	if (ret)
		ISP_ERR("VIDIOC_VIN_TDM_MAP error!\n");
	return ret;
}
...
int isp_dev_requset_tdm_data(struct hw_isp_device *isp, struct vin_isp_tdm_data *data)
{
	...
	ret = ioctl(isp->tdm_rx.fd, VIDIOC_VIN_TDM_REQ_DATA, data);
	if (ret)
		ISP_ERR("VIDIOC_VIN_TDM_REQ_DATA error!\n");
	return ret;
}
...
int isp_dev_send_tdm_data(struct hw_isp_device *isp, struct vin_isp_tdm_data *data)
{
	...
	ret = ioctl(isp->tdm_rx.fd, VIDIOC_VIN_TDM_SEND_DATA, data);
	if (ret)
		ISP_ERR("VIDIOC_VIN_TDM_SEND_DATA(%d) error! ret=%d\n", VIDIOC_VIN_TDM_SEND_DATA, ret);
	return ret;
}
...
int isp_sensor_set_exp_gain(struct hw_isp_device *isp, struct sensor_exp_gain *exp_gain)
{
	...
	if (memcmp(&exp_gain_save[isp->id], exp_gain, sizeof(struct sensor_exp_gain)) || cnt > 10) {
		cnt = 0;
...
{
	int ret;
	ret = ioctl(isp->sensor.fd, VIDIOC_VIN_SET_SOFTWARE_TDM_CHANGE, change_flags);
	if (ret < 0)
		ISP_ERR("%s set software tdm change failed: %s (%d).\n",
			isp->sensor.info.name, strerror(errno), errno);
	return ret;
}
...
{
	int ret = 0;
	ret = ioctl(isp->sensor.fd, VIDIOC_VIN_SENSOR_GET_FLIP, flip);
	if (ret < 0)
		ISP_WARN("%s get sensor_flip failed: %s (%d).\n",
			isp->sensor.info.name, strerror(errno), errno);
	return ret;
}
...
int isp_sensor_mipi_switch(struct hw_isp_device *isp, struct sensor_mipi_switch_entity *sensor_mipi_switch_info)
{
	...
	ret = ioctl(isp->sensor.fd, VIDIOC_VIN_SENSOR_MIPI_SWITCH, sensor_mipi_switch_info);
	if (ret < 0)
		ISP_WARN("%s VIDIOC_VIN_SENSOR_MIPI_SWITCH failed: %s (%d).\n",
			isp->sensor.info.name, strerror(errno), errno);
	return ret;
}
